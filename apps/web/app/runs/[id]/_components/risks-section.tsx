...
const LEVEL_LABEL: Record<Risk["likelihood"], string> = { low: "низький", medium: "середній", high: "високий" };
...
export function RisksSection({ register }: { register: RiskRegister | undefined }) {
  ...
  return (
    <SectionShell
      ...
      description="Суттєві ризики з імовірністю, впливом і конкретною мітигацією."
    >
      {risks.length === 0 ? (
        <EmptyState message="Ризиків поки не визначено." />
      ) : (
        <div className="overflow-hidden rounded-md border border-border/60">
          <table className="w-full text-sm">
            <thead className="bg-muted/30 text-[10px] uppercase tracking-[0.14em] text-muted-foreground">
              <tr>
                <th className="px-3 py-2 text-left font-medium">Ризик</th>
                <th className="px-3 py-2 text-left font-medium">Імовірність</th>
                <th className="px-3 py-2 text-left font-medium">Вплив</th>
                <th className="px-3 py-2 text-left font-medium">Мітигація</th>
              </tr>
            </thead>
            <tbody>
              {risks.map((r, i) => (
